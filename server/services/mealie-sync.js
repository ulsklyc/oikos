/**
 * Modul: Mealie-Sync
 * Zweck: Rezepte einer selbst gehosteten Mealie-Instanz read-only in die lokale
 *        recipes/recipe_ingredients-Tabelle spiegeln. Mealie bleibt Quelle der
 *        Wahrheit für Rezeptinhalte; hier passiert nur Import, nie Schreibzugriff
 *        zurück nach Mealie.
 *
 ...
 *
 * Dependencies: server/db.js, ./mealie/client.js
 */
import { createLogger } from '../logger.js';
import * as db from '../db.js';
import { MealieAdapter, flattenIngredient } from './mealie/client.js';

const log = createLogger('MealieSync');
...
let adapterFactory = (account) => new MealieAdapter(account);
/** Test-Hook: injiziert einen Fake-Adapter statt echter HTTP-Requests. */
export function _setAdapterFactory(fn) { adapterFactory = fn || ((account) => new MealieAdapter(account)); }
export function getAdapter(account) { return adapterFactory(account); }

function getEnabledAccounts() {
  return db.get().prepare('SELECT * FROM mealie_accounts WHERE enabled = 1').all();
}
 ...
 * nachladen, upserten, dann lokale Mirror-Rezepte löschen, die Mealie nicht
...
async function syncAccount(account) {
  ...
  for (const summary of summaries) {
    ...
    // jeder Rezept-Umbenennung in Mealie neu aus dem Namen abgeleitet. Mit dem
    // Slug als Schlüssel würde ein Umbenennen als Löschen+Neuanlegen erscheinen
    // und jede Essensplan-Verknüpfung zu diesem Rezept stumm verlieren
    // (meals.recipe_id ON DELETE SET NULL). Der Slug wird weiterhin für den
    ...
    let detail;
    try {
      detail = await adapter.getRecipe(summary.slug);
    } catch (err) {
      log.error(`Account ${account.id}: failed to fetch recipe "${summary.slug}":`, err.message);
      continue; // nur dieses eine Rezept überspringen, Rest des Laufs weitermachen
    }

    const recipeUrl = adapter.recipeUrl(groupSlug, detail.slug);
    const ingredients = (detail.recipeIngredient || []).map(flattenIngredient);
    toUpsert.push({ current, detail, recipeUrl, ingredients });
  }

  // Leer-Guard: existieren lokal Mirror-Rezepte, aber Mealie liefert diesmal gar
  // keine Summaries, ist das eher ein stiller Server-/Auth-Fehler als eine
  // tatsächlich geleerte Sammlung - nicht löschen (vgl. calendar-prune.js).
  let stale = [];
  if (summaries.length > 0 || existing.length === 0) {
    stale = existing.filter((r) => !seenIds.has(r.mealie_recipe_id));
  } else if (existing.length > 0) {
    log.warn(`Account ${account.id} ("${account.name}"): Mealie returned no recipes, but ${existing.length} exist locally. Skipping deletion.`);
  }
...
function recordAccountResult(account, result) {
  if (result.failed) {
    db.get().prepare('UPDATE mealie_accounts SET last_error = ? WHERE id = ?').run(result.error, account.id);
    return false;
  }
  db.get().prepare('UPDATE mealie_accounts SET last_sync = ?, last_error = NULL WHERE id = ?')
    .run(new Date().toISOString(), account.id);
  return true;
}

/** Sync-Durchlauf über alle aktivierten Accounts. */
export async function sync() {
  const accounts = getEnabledAccounts();
  if (accounts.length === 0) {
    log.debug('No enabled Mealie accounts configured.');
  ...
  const summary = `Mealie sync complete: ${successfulAccounts}/${accounts.length} accounts, `
...
export async function syncOne(accountId) {
  ...
  if (!account) throw new Error('Mealie account not found.');
...
export function getStatus() {
  const accounts = db.get().prepare('SELECT * FROM mealie_accounts ORDER BY name COLLATE NOCASE').all();
  const counts = db.get().prepare(`
    SELECT mealie_account_id, COUNT(*) AS c FROM recipes
    WHERE mealie_account_id IS NOT NULL GROUP BY mealie_account_id
  `).all();
  const countByAccount = new Map(counts.map((row) => [row.mealie_account_id, row.c]));
...
export function startScheduler() {
  const run = () => {
    sync().catch((err) => log.error('Mealie sync scheduler run failed:', err?.message || err));
  };
  setTimeout(run, 10_000).unref();
  setInterval(run, SYNC_INTERVAL_MS).unref();
  log.info('Mealie sync scheduler active (every 60 min).');
}
